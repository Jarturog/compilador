...
public class Optimizador {
    ...
    public Optimizador(CodeGeneration3Address cg) {
        codigo = new ArrayList<>(cg.getInstruccions());
    }
    ...
    public boolean brancamentAdjacent() {
        ...
        for (int i = 0; i < codigo.size(); i++) {
            if (isIf(codigo.get(i).getTipoInstr())) { //if cond goto e1
                //guardo la etiqueta
                Operador e1 = codigo.get(i).getOperators()[2];
                int posIf = i;
                i++;
                if (i < codigo.size() && codigo.get(i).getTipoInstr() == TipoInstr.GOTO) { //goto e2
                    //guardamos la etiqueta del goto
                    Operador e2 = codigo.get(i).getOperators()[2];
                    int posGoto = i;
                    i++;
                    if (i < codigo.size() && codigo.get(i).getTipoInstr() == TipoInstr.SKIP && codigo.get(i).getOperators()[2].getLabel().equals(e1.getLabel())) { //e1 : skip

                        codigo.get(posIf).setTipoInstr(negCond(codigo.get(posIf).getTipoInstr()));
                        ...
                        for (int j = 0; j < codigo.size(); j++) {
                            if ((isIf(codigo.get(j).getTipoInstr()) || codigo.get(j).getTipoInstr() == TipoInstr.GOTO) && codigo.get(j).getOperators()[2].getLabel().equals(e1.getLabel())) {
                                borrar = false;
                                break;
                            }
                        }
